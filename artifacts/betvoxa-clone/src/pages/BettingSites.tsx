        ...
        <div className="flex flex-col gap-6">
          {sportsbooks.map((site, i) => (
            <motion.div key={site.name}
              ...
              className={`bg-[#111009] border rounded-2xl p-6 transition-all duration-300 ${site.featured ? "border-[#D97706]/25" : "border-white/8"}`}>
              {site.featured && (
                <div className="inline-flex items-center gap-1.5 bg-[#D97706]/10 border border-[#D97706]/30 text-[#D97706] text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full mb-4">
                  <Star size={9} className="fill-[#D97706]" /> Editor's Choice
                </div>
              )}
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#D97706]/20 to-[#1B3950]/40 border border-white/10 flex items-center justify-center font-bold text-[#D97706] text-xl font-mono mb-3">{site.initials}</div>
                  <div className="flex mb-1">{Array.from({ length: 5 }).map((_, j) => <Star key={j} size={12} className={j < Math.round(site.rating) ? "fill-[#D97706] text-[#D97706]" : "fill-white/20 text-white/20"} />)}</div>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mb-4">
                    {site.features.map((f) => <div key={f} className="flex items-center gap-2 text-sm text-white/60"><Check size={12} className="text-[#D97706] flex-shrink-0" />{f}</div>)}
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {methodologyPoints.map((p, i) => (
              <motion.div key={p.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                ...
                <div className="w-10 h-10 rounded-lg bg-[#D97706]/10 flex items-center justify-center flex-shrink-0"><p.icon size={18} className="text-[#D97706]" /></div>
                <div>
                  ...
                  return (
                    <tr key={row.event} className={`border-b border-white/5 ${i % 2 === 0 ? "bg-[#0e0c0a]" : "bg-[#111009]"}`}>
                      ...
                      {[row.bet365, row.draftkings, row.fanDuel, row.betfair].map((odd, j) => (
                        <td key={j} className={`text-center px-5 py-3.5 font-medium ${parseFloat(odd) === best ? "text-[#D97706] font-bold" : "text-white/60"}`}>{odd}</td>
                      ))}
                    </tr>
                  );
                })}
                ...
                <tr key={row.sport} className={`border-b border-white/5 ${i % 2 === 0 ? "bg-[#0e0c0a]" : "bg-[#111009]"}`}>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2 flex-wrap">
                      {row.sites.map((s) => <span key={s} className="bg-[#D97706]/10 border border-[#D97706]/20 text-[#D97706] text-xs px-2 py-0.5 rounded-full">{s}</span>)}
                    </div>
                  </td>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="flex flex-col gap-2">
                {["Watch the game — don't bet blind", "Use cash out to manage risk", "Odds move fast — be decisive", "Pre-match research gives you an edge"].map((tip) => (
                  <div key={tip} className="flex items-center gap-2 text-sm text-white/60"><Check size={12} className="text-[#D97706]" />{tip}</div>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <tbody>
              {[
                ...
                <tr key={row.method} className={`border-b border-white/5 ${i % 2 === 0 ? "bg-[#0e0c0a]" : "bg-[#111009]"}`}>
                  ...
                  <td className={`px-5 py-3.5 text-xs font-medium ${row.bonus === "Yes" ? "text-[#D97706]" : "text-white/40"}`}>{row.bonus}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex flex-col gap-3">
            {faqs.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className={`bg-[#111009] border rounded-xl overflow-hidden transition-all ${openFaq === i ? "border-[#D97706]/30" : "border-white/8"}`}>
