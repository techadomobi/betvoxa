  ...
  return (
    <div className="min-h-screen bg-[#F8F7F2]">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-3">Expert Reviews</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="text-2xl font-bold text-[rgb(249, 115, 22)] mb-1">{stat.value}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <h2 className="font-serif text-2xl font-bold text-[#1F1A17] mb-6">Top Recommended Sportsbooks</h2>
        <div className="flex flex-col gap-6">
          {sportsbooks.map((site, i) => (
            <motion.div key={site.name}
              ...
              className={`bg-white border rounded-2xl p-6 transition-all duration-300 ${site.featured ? "border-[rgb(249, 115, 22)]/12" : "border-[#ECE6DB]"}`}>
              {site.featured && (
                <div className="inline-flex items-center gap-1.5 bg-[rgb(249, 115, 22)]/8 border border-[rgb(249, 115, 22)]/12 text-[rgb(249, 115, 22)] text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full mb-4">
                  <Star size={9} className="fill-[rgb(249, 115, 22)]" /> Editor's Choice
                </div>
              )}
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[rgb(249, 115, 22)]/20 to-[#1B3950]/40 border border-[#E7E1D6] flex items-center justify-center font-bold text-[rgb(249, 115, 22)] text-xl font-mono mb-3">{site.initials}</div>
                  <div className="flex mb-1">{Array.from({ length: 5 }).map((_, j) => <Star key={j} size={12} className={j < Math.round(site.rating) ? "fill-[rgb(249, 115, 22)] text-[rgb(249, 115, 22)]" : "fill-white/20 text-[#C9C3B8]"} />)}</div>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mb-4">
                    {site.features.map((f) => <div key={f} className="flex items-center gap-2 text-sm text-[#5F554C]"><Check size={12} className="text-[rgb(249, 115, 22)] flex-shrink-0" />{f}</div>)}
                  </div>
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-green-400 font-bold mb-1">Pros</div>
                      {site.pros.map((p) => <div key={p} className="text-xs text-[#5F554C] flex items-center gap-1"><span className="text-green-400">+</span>{p}</div>)}
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-red-400 font-bold mb-1">Cons</div>
                      {site.cons.map((c) => <div key={c} className="text-xs text-[#5F554C] flex items-center gap-1"><span className="text-red-400">−</span>{c}</div>)}
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-2 flex-shrink-0 justify-start md:items-end">
                  <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                    className="flex items-center gap-2 px-5 py-2.5 bg-[rgb(249, 115, 22)] text-[#0B0A09] rounded-lg font-bold text-sm hover:bg-[#DC6803] transition-colors shadow-[0_0_16px_rgba(249,115,22,0.2)]"
                    data-testid={`button-visit-${site.name.toLowerCase().replace(/\s+/g, "-")}`}>
                    Visit Site <ExternalLink size={13} />
                  </motion.button>
                  <button className="flex items-center gap-2 px-5 py-2.5 border border-[#DAD3C6] text-[#4A433C] rounded-lg font-medium text-sm hover:border-[#CFC7BB] hover:text-[#1F1A17] transition-colors"
      ...
      <section className="bg-[#F3F1EA] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-3">Our methodology</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {methodologyPoints.map((p, i) => (
              <motion.div key={p.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                ...
                <div className="w-10 h-10 rounded-lg bg-[rgb(249, 115, 22)]/8 flex items-center justify-center flex-shrink-0"><p.icon size={18} className="text-[rgb(249, 115, 22)]" /></div>
                <div>
                  <div className="flex items-baseline gap-2 mb-1">
                    <span className="font-semibold text-[#1F1A17]">{p.title}</span>
                    <span className="text-[rgb(249, 115, 22)] text-sm font-bold">{p.pct}</span>
                  </div>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-3">Start here</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bettingGuide.map((s, i) => (
            <motion.div key={s.step} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              ...
              <div className="absolute -top-3 -left-3 w-8 h-8 rounded-full bg-[rgb(249, 115, 22)] text-[#0B0A09] font-bold text-sm flex items-center justify-center shadow-lg">{s.step}</div>
      ...
      <section className="py-20 bg-[#F3F1EA]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-3">Find the best price</div>
          ...
          <div className="overflow-x-auto rounded-xl border border-[#ECE6DB]">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white border-b border-[#ECE6DB]">
                  <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Event / Selection</th>
                  ...
                  <th className="text-center px-5 py-3 text-[rgb(249, 115, 22)] font-medium">Betfair</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={row.event} className={`border-b border-[#EFE9DE] ${i % 2 === 0 ? "bg-[#FAF8F3]" : "bg-white"}`}>
                      ...
                      {[row.bet365, row.draftkings, row.fanDuel, row.betfair].map((odd, j) => (
                        <td key={j} className={`text-center px-5 py-3.5 font-medium ${parseFloat(odd) === best ? "text-[rgb(249, 115, 22)] font-bold" : "text-[#5F554C]"}`}>{odd}</td>
                      ))}
                    </tr>
                  );
                })}
          ...
          <p className="text-[#B4ADA3] text-xs mt-3 text-center">Odds are indicative and may change. Betfair exchange often offers best prices due to peer-to-peer betting.</p>
        </div>
      </section>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <h2 className="font-serif text-4xl font-bold text-[#1F1A17] mb-4">Sports Coverage Guide</h2>
          <p className="text-[#6F665D]">Find the best sportsbook for your favourite sport.</p>
        </motion.div>
        <div className="overflow-x-auto rounded-xl border border-[#ECE6DB]">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white border-b border-[#ECE6DB]">
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Sport</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Top Sites</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Markets</th>
              </tr>
            </thead>
            <tbody>
              {sportsGrid.map((row, i) => (
                <tr key={row.sport} className={`border-b border-[#EFE9DE] ${i % 2 === 0 ? "bg-[#FAF8F3]" : "bg-white"}`}>
                  <td className="px-5 py-3.5 text-[#1F1A17] font-medium">{row.sport}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2 flex-wrap">
                      {row.sites.map((s) => <span key={s} className="bg-[rgb(249, 115, 22)]/8 border border-[rgb(249, 115, 22)]/12 text-[rgb(249, 115, 22)] text-xs px-2 py-0.5 rounded-full">{s}</span>)}
                    </div>
                  </td>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-3">Real-time action</div>
              ...
              <div className="flex flex-col gap-2">
                {["Watch the game — don't bet blind", "Use cash out to manage risk", "Odds move fast — be decisive", "Pre-match research gives you an edge"].map((tip) => (
                  <div key={tip} className="flex items-center gap-2 text-sm text-[#5F554C]"><Check size={12} className="text-[rgb(249, 115, 22)]" />{tip}</div>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
              className="bg-white border border-[#ECE6DB] rounded-2xl p-6">
              <h3 className="font-semibold text-[#1F1A17] mb-4">Best Sites for Live Betting</h3>
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <div key={item.name} className="flex items-center gap-3 py-2 border-b border-[#EFE9DE] last:border-0">
                    <div className="w-8 h-8 rounded-lg bg-[rgb(249, 115, 22)]/15 flex items-center justify-center text-[rgb(249, 115, 22)] text-xs font-bold">{item.name.slice(0, 2)}</div>
                    <div>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <h2 className="font-serif text-4xl font-bold text-[#1F1A17] mb-4">Payment Methods Comparison</h2>
          <p className="text-[#6F665D] max-w-xl mx-auto">Choose the right payment method for fast deposits and even faster withdrawals.</p>
        </motion.div>
        <div className="overflow-x-auto rounded-xl border border-[#ECE6DB]">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white border-b border-[#ECE6DB]">
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Method</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Deposit Speed</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Withdrawal Speed</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Fee</th>
                <th className="text-left px-5 py-3 text-[#5F554C] font-medium">Bonus Eligible</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={row.method} className={`border-b border-[#EFE9DE] ${i % 2 === 0 ? "bg-[#FAF8F3]" : "bg-white"}`}>
                  <td className="px-5 py-3.5 text-[#1F1A17] font-medium">{row.method}</td>
                  ...
                  <td className={`px-5 py-3.5 text-xs font-medium ${row.bonus === "Yes" ? "text-[rgb(249, 115, 22)]" : "text-[#8D847A]"}`}>{row.bonus}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex flex-col gap-3">
            {faqs.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className={`bg-white border rounded-xl overflow-hidden transition-all ${openFaq === i ? "border-[rgb(249, 115, 22)]/12" : "border-[#ECE6DB]"}`}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  ...
                  {openFaq === i ? <ChevronUp size={15} className="text-[rgb(249, 115, 22)] flex-shrink-0" /> : <ChevronDown size={15} className="text-[#8D847A] flex-shrink-0" />}
                </button>
                <AnimatePresence>
                  {openFaq === i && (
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <Shield size={32} className="text-[rgb(249, 115, 22)] mx-auto mb-4" />
