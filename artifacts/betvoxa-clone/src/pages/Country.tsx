        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {data.topSites.map((site, i) => (
              ...
              >
                <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-[#D97706]/20 to-[#1B3950]/40 border border-[#E7E1D6] flex items-center justify-center font-bold text-[#D97706] text-sm font-mono flex-shrink-0">
                  {site.initials}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex mt-1">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <span key={j} className={`text-[10px] ${j < Math.round(site.rating) ? "text-[#D97706]" : "text-[#C9C3B8]"}`}>★</span>
                    ))}
                  </div>
                </div>
                <motion.button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3">
            {data.paymentMethods.map((method) => (
              <span
                key={method}
                className="bg-white border border-[#E7E1D6] text-[#4A433C] px-4 py-2 rounded-lg text-sm hover:border-[#D97706]/12 transition-colors"
              >
                {method}
              </span>
            ))}
          ...
          {Object.entries(countryData)
            ...
            .map(([key, country]) => (
              <Link key={key} href={`/country/${key}`}>
                <motion.div
                  whileHover={{ scale: 1.04 }}
                  className="flex items-center gap-2 bg-white border border-[#ECE6DB] hover:border-[#D97706]/12 rounded-lg px-4 py-2.5 cursor-pointer transition-all"
