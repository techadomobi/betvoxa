        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <div className="text-[rgb(249, 115, 22)] text-sm font-semibold uppercase tracking-widest mb-1">Country Guide</div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            >
              <div className="text-[rgb(249, 115, 22)] text-xs font-semibold uppercase tracking-wider mb-1">{item.label}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {data.topSites.map((site, i) => (
              ...
              >
                <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-[rgb(249, 115, 22)]/20 to-[#1B3950]/40 border border-[#E7E1D6] flex items-center justify-center font-bold text-[rgb(249, 115, 22)] text-sm font-mono flex-shrink-0">
                  {site.initials}
                </div>
                <div className="flex-1">
                  <div className="font-semibold text-[#1F1A17]">{site.name}</div>
                  <div className="text-[rgb(249, 115, 22)] text-sm">{site.bonus}</div>
                  <div className="flex mt-1">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <span key={j} className={`text-[10px] ${j < Math.round(site.rating) ? "text-[rgb(249, 115, 22)]" : "text-[#C9C3B8]"}`}>★</span>
                    ))}
                  </div>
                </div>
                <motion.button
                  whileHover={{ scale: 1.04 }}
                  className="px-4 py-2 bg-[rgb(249, 115, 22)] text-[#0B0A09] rounded-lg text-sm font-bold flex items-center gap-1 hover:bg-[#DC6803] transition-colors"
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <Shield size={20} className="text-[rgb(249, 115, 22)]" />
            ...
            <div className="flex flex-col gap-3">
              {data.tips.map((tip) => (
                <div key={tip} className="flex items-start gap-3">
                  <Check size={15} className="text-[rgb(249, 115, 22)] mt-0.5 flex-shrink-0" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3">
            {data.paymentMethods.map((method) => (
              <span
                key={method}
                className="bg-white border border-[#E7E1D6] text-[#4A433C] px-4 py-2 rounded-lg text-sm hover:border-[rgb(249, 115, 22)]/12 transition-colors"
              >
                {method}
              </span>
            ))}
          ...
          {Object.entries(countryData)
            ...
            .map(([key, country]) => (
              <Link key={key} href={`/country/${key}`}>
                <motion.div
                  whileHover={{ scale: 1.04 }}
                  className="flex items-center gap-2 bg-white border border-[#ECE6DB] hover:border-[rgb(249, 115, 22)]/12 rounded-lg px-4 py-2.5 cursor-pointer transition-all"
